...
使用服务器资源包和数据库存储来在线加载与播放URL中的音乐，无需配置客户端,允许管理员配置服务器预设的音乐！支持播放预设歌曲或流式播放在线音乐，创建共享音乐室以实现同步收听，并通过直观的指令和GUI界面管理播放。利用资源包提供无缝的音频体验。
...
- [⚙️ 配置文件 (`config.yml`)](#️-配置文件-configyml)
...
```yaml
resourcePack:
  packFormat: 32 # 资源包格式版本 (Minecraft 1.20.5 - 1.20.6 使用 26, 1.21 使用 32)。请根据你的服务器版本调整。
...
messages:
  ...
  bf: # /bf 指令相关的消息
    ...
    # ... (其他 bf 子命令的消息，例如 play, playurl, stop, createroom 等，已在你提供的内容中) ...
  playurl: # /playurl 独立指令相关的消息
    usage: "§6用法: /playurl <URL>"
    preparing: "§e正在准备音乐资源包..."
    packCreationFailed: "§c创建资源包失败。"
    error: "§c播放音乐时发生错误。"
  resourcePack: # 资源包状态相关的消息
    status:
      accepted: "§a资源包已接受！"
...
| `eogdmusicplayer.internal.join`      | 允许内部机制加入房间 (例如通过其他插件功能)。| `op`     |

## 常见问题 (FAQ)
*   **Q: 为什么玩家听不到音乐/资源包提示不出现或失败？**
    A: 请检查以下几点：
...
*   **Q: 如何添加我自己的预设歌曲？**
    A: 编辑 `plugins/EogdMusicPlayer/config.yml` 文件中的 `presetSongs` 部分。按照已有歌曲的格式添加新的条目，指定 `name` (显示名称), `url` (音乐链接), 和 `item` (GUI中的图标)。修改后使用 `/bf reload` 重载配置。

*   **Q: `resourcePack.packFormat` 应该设置成多少？**
    A: 这取决于你的服务器 Minecraft 版本。常见的有：
        *   1.20.2: 18
        *   1.20.3 - 1.20.4: 22
        *   1.20.5 - 1.20.6: 26
        *   **1.21: 32** (根据你的 `api-version: '1.21'`, 这个值应该是 `32`)
...
2.  创建一个新的**特性分支** (`git checkout -b feature/YourAmazingFeature`)。
